...
class phpthumb_filters {
	...
	function Colorize(&$gdimg, $amount, $targetColor) {
		...
		if (phpthumb_functions::version_compare_replacement(phpversion(), '5.0.0', '>=') && phpthumb_functions::gd_is_bundled()) {
			...
			$r = hexdec(substr($targetColor, 0, 2));
			$g = hexdec(substr($targetColor, 2, 2));
			$b = hexdec(substr($targetColor, 4, 2));
			...
			// end background color block
				...
				foreach ($textlines as $dummy => $line) {
					...
					switch ($alignment) {
						...
						case 'T':
						case 'B':
							$text_origin_x = round((ImageSX($gdimg) - $text_width_line) / 2);
							break;

						case 'R':
						case 'TR':
						case 'BR':
							$text_origin_x = ImageSX($gdimg) - $text_width_line  + $TTFboxLine[0] - $min_x_line + round($size * 0.25) - $margin;
							break;
					}
